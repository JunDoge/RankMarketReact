...
function PrdReview() {
    const [prdreviews, setPrdReviews] = useState([]);
    ...
    useEffect(() => {
        axios.get('/mypage/history/review',{
            ...
            setPrdReviews(prevReviews => [...prevReviews, ...newReviews])
        }).catch(e =>{
            navigate('/error')
        })
    ...
    const delReview = (prd_id) => {
        axios.delete('/delete/reviews',null,{
            ...
            setPrdReviews(prdreviews => prdreviews.filter(review => review.prdId !== prd_id));
            setShowModal(true);
            setTimeout(() => {
                setShowModal(false);
            }, 2000);
    ...
    return(
        ...
        <Row className="mb-2">
    <Col xs={4} className="d-flex align-items-center justify-content-center"><strong>평점</strong></Col>
    <Col xs={2} className="d-flex align-items-center justify-content-center"><strong>이미지</strong></Col>
    <Col xs={5} className="d-flex align-items-center justify-content-center"><strong>리뷰내용</strong></Col>
</Row>

            <div className="borderBottom"></div>
            {prdreviews.map((review, index) => (
                <div key={index}>
                    <Row>
                        <Col xs={4} className="d-flex align-items-center justify-content-center font-weight-bold font20">
                            <StarRating rating={review.rate_scr} />
                        </Col>
